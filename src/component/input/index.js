...
export default (props, key) => {
  switch (props.type) {
    ...
    case 'textInput':
      return (
        <View key={key} onLayout={val => props.onSetLayout && props.onSetLayout(val.nativeEvent)} style={{ paddingHorizontal: 24, display: props.inVisible ? 'none' : 'flex', ...props.style }}>
          <Imaterial {...props} />
          {props.required ? <Star /> : null}
        </View>
      )
    ...
    case 'radioColumn':
      return (
        <View key={key} onLayout={val => props.onSetLayout && props.onSetLayout(val.nativeEvent)} style={{ marginTop: 16, display: props.inVisible ? 'none' : 'flex' }}>
          <IradioColumn {...props} />
          {props.required ? <Star /> : null}
        </View>
      )
    ...
    case 'dateExpire':
      return (
        <View key={key} onLayout={val => props.onSetLayout && props.onSetLayout(val.nativeEvent)} style={{ paddingHorizontal: 24, marginTop: 16, display: props.inVisible ? 'none' : 'flex' }}>
    ...
    case 'modal':
      return (
        <View key={key} onLayout={val => props.onSetLayout && props.onSetLayout(val.nativeEvent)} style={{ paddingHorizontal: 24, marginTop: 10, display: props.inVisible ? 'none' : 'flex' }}>
          <TLight fontSize="14" mb="10" mt="10" textAlign="left" color={props.err ? 'rgb(213, 0, 0)' : colors.grey}>{props.label}</TLight>
          <InputModal {...props} />
          {props.required ? <Star /> : null}
